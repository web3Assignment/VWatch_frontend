# VWatch Frontend Codebase Context & UI/UX Redesign Handover Guide

This document provides a comprehensive overview of the **VWatch** frontend client codebase (Vite + React + TypeScript + Tailwind CSS v4). It details the system architecture, design systems, state stores, WebSocket synchronization flows, page structures, and components. 

Use this document to instruct another AI agent (e.g., Claude) to redesign or enhance the UI/UX. The core rule is: **Modify the styling, animations, colors, layouts, and typography, but DO NOT modify, delete, or break any existing features, states, hooks, handlers, or WebSocket event flows.**
...
│   ├── dashboard/  # CreateRoomModal.tsx, JoinRoomModal.tsx, RoomCard.tsx
│   ├── room/       # ChatPanel.tsx, VideoPlayer.tsx, ParticipantList.tsx, SyncTelemetry.tsx, etc.
│   └── shared/     # Button.tsx, GlassCard.tsx, Input.tsx, Navbar.tsx, Modal.tsx, RoleBadge.tsx, etc.
├── config/         # Environment config (env.ts)
├── hooks/          # React custom hooks wrapping stores (useAuth, useRoom, usePlayer, useChat, etc.)
├── pages/          # Page views rendered by router (Landing, Dashboard, Room, Explore, Premium, Account, Showcase)
├── router/         # React Router DOM v7 configuration (index.tsx)
├── services/       # Mock service layer (api.client.ts, auth.service.ts, room.service.ts, socket.service.ts)
├── store/          # Zustand global state management (useAuthStore.ts, useRoomStore.ts, useThemeStore.ts)
├── types/          # TypeScript interface contracts (user, room, participant, player, chat)
└── utils/          # Helper utilities (youtube.ts, cookie.ts, room.ts)
```
...
## 🧠 State Management & Websocket Listeners (DO NOT BREAK)

VWatch depends on three global **Zustand stores** (`src/store/`) exposed through unified hooks (`src/hooks/`). The AI agent MUST keep these exact stores and listeners intact.
...
When rewriting markup for the pages, verify all interactive callbacks (`onClick`, `onSubmit`, input bindings `value`, list `.map()`, loading wrappers) are retained exactly as implemented.
...
This is the core collaborative view. It is structured into multiple key UI components that must not be broken during redesign:
...
*   **YouTube Player Container**: 
    *   **Playback Blocker Overlay**: When `!canControl` (user is a Viewer or Participant without host permissions), an invisible `<div className="absolute inset-0 z-30 bg-transparent cursor-not-allowed">` blocks clicks on the YouTube player frame. This prevents viewers from manually scrubbing or pausing the video out of sync. **Do not remove this div**.
*   **Control Panel**:
...
*   **YouTube Search Sidebar**:
    ...
    *   Fetches videos from YouTube API using the query. Clicking a search result changes the room's current active video using `changeVideo(videoId)`. **Note:** This sidebar must only allow selection if the user has control permissions (`canControl`).
...
## 🎨 Redesign Directives (For Claude / Redesign AI Agent)

When designing a new layout:
1.  **Refine CSS Variables**: You can update colors, background scales, blur indices, or shadow states inside `@theme` in `src/index.css`.
2.  **Enhance CSS Styles**: Add more premium layouts, glow-borders, custom cards, hover micro-interactions, and visual states.
3.  **Refactor Components Styling**: Re-write Tailwind utility class lists in pages and shared components (`src/components/shared/` and `src/pages/`) to implement a modern, responsive layout.
4.  **Preserve State Bindings**: Ensure you keep all variables like `isLoading`, `user`, `participants`, `playerState`, and `messages` active in components.
5.  **Preserve Event Callbacks**: Ensure forms still submit via their handlers (e.g., `handleSend` in `ChatPanel`) and buttons retain their click attributes (e.g., `onClick={() => handleSeekOffset(-10)}` in `VideoPlayer`).
6.  **Maintain Light/Dark Support**: Design rules should adjust to theme-toggles via CSS variables (e.g. using `var(--surface)` / `bg-surface`) to support both light and dark modes cleanly.
